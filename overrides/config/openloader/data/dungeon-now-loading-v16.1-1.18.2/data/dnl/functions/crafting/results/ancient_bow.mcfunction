...
execute if entity @s[nbt={Item:{id:"minecraft:bow",tag:{Enchantments:[{id:"minecraft:vanishing_curse",lvl:1s}]}}}] store success score @s dnl.success positioned ~ ~1.5 ~ as @e[type=minecraft:item_frame,tag=dnl.material,tag=dnl.bow,tag=dnl.vanishing_curse,tag=!dnl.material.used,distance=..2.1,sort=random,limit=1] run function dnl:crafting/material_used

### Reset Crafter ###
execute if score @s dnl.success matches 1.. run function dnl:crafting/success

### Crafted item ###
execute if score @s dnl.craftpoint matches 6 run loot spawn ~ ~ ~ loot dnl:items/ancient_bow
