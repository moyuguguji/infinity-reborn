...
execute if score @s dnl.timer matches 1 run data merge entity @s {HandItems:[{id:"minecraft:potion",Count:1b,tag:{Potion:"minecraft:jump_boost"}},{}]}
...
execute if score @s dnl.timer matches 30 run effect give @s minecraft:jump_boost 180 0
execute if score @s dnl.timer matches 30 run tag @s remove dnl.use_potion
execute if score @s dnl.timer matches 30 run tag @s remove dnl.jump_boost
