...
### Next Attacker ###
execute as @s[scores={dnl.clock=150}] run tag @e[type=minecraft:ghast,tag=dnl.guardian_ghast,tag=dnl.main,tag=dnl.awoken,limit=1,tag=!dnl.attacker,tag=!dnl.attacker,sort=random,distance=..80] add dnl.attacker
